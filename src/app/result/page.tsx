  ...
  if (!result || !personality) {
    return (
      <main className="min-h-screen flex items-center justify-center">
        <div className="text-dark-600 text-sm animate-pulse">加载中...</div>
      </main>
    );
  }

  return (
    <main className="min-h-screen pb-32 page-enter" ref={resultRef}>
      {/* Hero section */}
      <div className="relative pt-16 pb-12 px-6">
        ...
        <div className="relative max-w-lg mx-auto text-center">
          ...
          <div className="text-5xl mb-6">{personality.icon}</div>
      ...
      <div className="max-w-lg mx-auto px-6 mb-12">
        <div className="glass rounded-2xl p-6">
          <p className="text-sm text-dark-300 leading-[1.8]">{personality.description}</p>
        </div>
      </div>
      ...
      <div className="max-w-lg mx-auto px-6">
        <div className="mb-8">
          <h3 className="text-xs font-mono text-dark-600 tracking-[0.2em] uppercase mb-3">社交表现</h3>
          <p className="text-sm text-dark-300 leading-relaxed">{personality.socialBehavior}</p>
        </div>
        <div className="mb-8">
          <h3 className="text-xs font-mono text-dark-600 tracking-[0.2em] uppercase mb-3">恋爱表现</h3>
          <p className="text-sm text-dark-300 leading-relaxed">{personality.loveBehavior}</p>
        </div>
        <div className="mb-8">
          <h3 className="text-xs font-mono text-dark-600 tracking-[0.2em] uppercase mb-3">深层心理需求</h3>
          <p className="text-sm text-dark-300 leading-relaxed">{personality.deepNeed}</p>
        </div>

        <div className="glass rounded-2xl p-6 mb-8">
          <h3 className="text-xs font-mono text-dark-600 tracking-[0.2em] uppercase mb-3">最容易被谁治愈</h3>
          <p className="text-sm text-dark-300 leading-relaxed">{personality.healedBy}</p>
        </div>

        <div className="glass rounded-2xl p-6 mb-8">
          <h3 className="text-xs font-mono text-dark-600 tracking-[0.2em] uppercase mb-3">别人对你的误解</h3>
      ...
      <div className="max-w-lg mx-auto px-6 mt-16">
        ...
        <div className="flex gap-3">
          ...
          <button
            onClick={() => router.push("/")}
            className="flex-1 py-3.5 rounded-xl bg-gradient-to-r from-cold-dim to-cold-blue text-sm text-white/90 hover:opacity-90 transition-opacity"
          >
            再测一次
          </button>
        </div>
